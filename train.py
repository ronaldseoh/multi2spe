...
class MultiFacetTripletLoss(torch.nn.Module):
    ...
    def forward(self, query, positive, negative, pos_instance_weights=None, neg_instance_weights=None, loss_instance_weights=None):
        ...
        if self.loss_type == "bce":
            ...
            losses = torch.nn.functional.cross_entropy(distances_as_logits, labels, reduction='none')
            ...
            if neg_instance_weights is not None:
                negative_part *= neg_instance_weights

            losses = positive_part + negative_part
        else:
            losses = torch.nn.functional.relu(distance_positive - distance_negative + self.margin)

        if loss_instance_weights is not None:
            losses *= loss_instance_weights
            ...
            if self.use_multiple_losses:
                ...
                for i, l in enumerate(self.loss_list):
                    if type(self.query_facet_magnitude_layers[i]) is torch.nn.Linear:
                        source_embedding_magnitudes = self.query_facet_magnitude_layers[i](source_output.last_hidden_state[:, :self.hparams.num_facets, :].contiguous())
