#!/bin/bash
#SBATCH --job-name=scincl-wol_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy
#SBATCH -o sbatch_logs/stdout/scincl-wol_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy_%j.txt
#SBATCH -e sbatch_logs/stderr/scincl-wol_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy_%j.err
#SBATCH --ntasks=1
...
EXPERIMENT_ID_PREFIX=scincl-wol_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy
EXPERIMENT_DATE=`date +"%m-%d"`

python train.py --save_dir save_${EXPERIMENT_ID_PREFIX}_${EXPERIMENT_DATE} \
                --train_file ~/my_696ds-s21/scincl_dataset_wol/train_triples.csv --train_metadata_file ~/my_696ds-s21/scincl_dataset_wol/train_metadata.jsonl --train_file_from_scincl --train_size 684100 \
