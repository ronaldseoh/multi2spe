#!/bin/bash
#SBATCH --job-name=k-3_original+max-individual_word-0-1_extra_facet_alternate_common_random_cross_entropy
#SBATCH -o sbatch_logs/stdout/k-3_original+max-individual_word-0-1_extra_facet_alternate_common_random_cross_entropy_%j.txt
#SBATCH -e sbatch_logs/stderr/k-3_original+max-individual_word-0-1_extra_facet_alternate_common_random_cross_entropy_%j.err
#SBATCH --ntasks=1
...
EXPERIMENT_ID_PREFIX=k-3_original+max-individual_word-0-1_extra_facet_alternate_common_random_cross_entropy
...
python train.py --save_dir save_${EXPERIMENT_ID_PREFIX}_${EXPERIMENT_DATE} \
                ...
                --loss_config '[{"name": "original", "weight": 1, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "max", "use_target_token_embs": false}, {"name": "max_and_individual_word_emb", "weight": 0.1, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "max", "reduction_multifacet_target": "mean", "reduction_multifacet_query_first": true, "use_target_token_embs": true, "do_not_use_target_token_embs_mean": true}]' \
