#!/bin/bash
#SBATCH --job-name=k-3_original+mean-avg_word-0-2+mean-mean-0-1_extra_facet_alternate_common_random_cross_entropy
#SBATCH -o sbatch_logs/stdout/k-3_original+mean-avg_word-0-2+mean-mean-0-1_extra_facet_alternate_common_random_cross_entropy_%j.txt
#SBATCH -e sbatch_logs/stderr/k-3_original+mean-avg_word-0-2+mean-mean-0-1_extra_facet_alternate_common_random_cross_entropy_%j.err
#SBATCH --ntasks=1
...
EXPERIMENT_ID_PREFIX=k-3_original+mean-avg_word-0-2+mean-mean-0-1_extra_facet_alternate_common_random_cross_entropy
...
python train.py --save_dir save_${EXPERIMENT_ID_PREFIX}_${EXPERIMENT_DATE} \
                ...
                --loss_config '[{"name": "original", "weight": 1, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "max", "use_target_token_embs": false}, {"name": "mean_and_word_emb", "weight": 0.2, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "mean", "use_target_token_embs": true}, {"name": "mean_and_mean", "weight": 0.1, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "mean", "use_target_token_embs": false}]' \
