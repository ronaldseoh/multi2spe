#!/bin/bash
#SBATCH --job-name=k-3_sum_embs_original-0-9+no_sum-0-1+mean-avg_word-0-05_extra_facet_alternate_layer_8_4-alternate_identity_common_random_instance_weights-3_cross_entropy
#SBATCH -o sbatch_logs/stdout/k-3_sum_embs_original-0-9+no_sum-0-1+mean-avg_word-0-05_extra_facet_alternate_layer_8_4-alternate_identity_common_random_instance_weights-3_cross_entropy_%j.txt
#SBATCH -e sbatch_logs/stderr/k-3_sum_embs_original-0-9+no_sum-0-1+mean-avg_word-0-05_extra_facet_alternate_layer_8_4-alternate_identity_common_random_instance_weights-3_cross_entropy_%j.err
#SBATCH --ntasks=1
...
EXPERIMENT_ID_PREFIX=k-3_sum_embs_original-0-9+no_sum-0-1+mean-avg_word-0-05_extra_facet_alternate_layer_8_4-alternate_identity_common_random_instance_weights-3_cross_entropy
...
python train.py --save_dir save_${EXPERIMENT_ID_PREFIX}_${EXPERIMENT_DATE} \
                ...
                --train_popularity_count_file ~/my_scratch/original_data/train_popularity_count.json \
                --val_popularity_count_file ~/my_scratch/original_data/val_popularity_count.json \
