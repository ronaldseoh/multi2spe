#!/bin/bash
#SBATCH --job-name=scincl-wol-readme_U_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy
#SBATCH -o sbatch_logs/stdout/scincl-wol-readme_U_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy_%j.txt
#SBATCH -e sbatch_logs/stderr/scincl-wol-readme_U_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy_%j.err
#SBATCH --ntasks=1
...
EXPERIMENT_ID_PREFIX=scincl-wol_U_k-3_debug_sum_embs_original-0-5+no_sum-0-5_extra_facet_alternate_layer_8_4-alternate_identity_common_random_cross_entropy
EXPERIMENT_DATE=`date +"%m-%d"`

python train.py --save_dir save_${EXPERIMENT_ID_PREFIX}_${EXPERIMENT_DATE} \
                --train_file /gypsum/scratch1/bseoh/new_readme_settings/train_triples.csv --train_metadata_file /gypsum/scratch1/bseoh/new_readme_settings/train_metadata.jsonl --train_file_from_scincl --train_size 684100 \
