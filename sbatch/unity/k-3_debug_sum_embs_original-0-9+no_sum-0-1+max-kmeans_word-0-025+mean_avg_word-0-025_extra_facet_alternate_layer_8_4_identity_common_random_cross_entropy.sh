#!/bin/bash
#SBATCH --job-name=U_k-3_debug_sum_embs_original-0-9+no_sum-0-1+max-kmeans_word-0-025+mean_avg_word-0-025_extra_facet_alternate_layer_8_4_identity_common_random_cross_entropy
#SBATCH -o sbatch_logs/stdout/U_k-3_debug_sum_embs_original-0-9+no_sum-0-1+max-kmeans_word-0-025+mean_avg_word-0-025_extra_facet_alternate_layer_8_4_identity_common_random_cross_entropy_%j.txt
#SBATCH -e sbatch_logs/stderr/U_k-3_debug_sum_embs_original-0-9+no_sum-0-1+max-kmeans_word-0-025+mean_avg_word-0-025_extra_facet_alternate_layer_8_4_identity_common_random_cross_entropy_%j.err
#SBATCH --ntasks=1
...
EXPERIMENT_ID_PREFIX=U_k-3_debug_sum_embs_original-0-9+no_sum-0-1+max-kmeans_word-0-025+mean_avg_word-0-025_extra_facet_alternate_layer_8_4_identity_common_random_cross_entropy
...
python train.py --save_dir save_${EXPERIMENT_ID_PREFIX}_${EXPERIMENT_DATE} \
                ...
                --loss_config '[{"name": "original", "weight": 0.9, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "max", "use_target_token_embs": false, "sum_into_single_embeddings": true}, {"name": "no_sum", "weight": 0.1, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "max", "use_target_token_embs": false, "sum_into_single_embeddings": false}, {"name": "max_and_word_emb_kmeans", "weight": 0.025, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "max", "use_target_token_embs": false, "use_target_token_embs_kmeans": true}, {"name": "mean_and_word_emb", "weight": 0.025, "loss_type": "bce", "margin": 1.0, "distance": "dot", "reduction": "mean", "reduction_multifacet": "mean", "use_target_token_embs": true}]' \
